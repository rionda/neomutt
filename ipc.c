...
int socket_create()
{
  ...
  if (listen(Socket.fd, 5) == -1)
    return -1;
  Socket.msg.data.attach = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.attach);
  Socket.msg.data.bcc = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.bcc);
  Socket.msg.data.cc = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.cc);
  Socket.msg.data.config = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.config);
  Socket.msg.data.command = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.command);
  Socket.msg.data.folder = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.folder);
  Socket.msg.data.postponed = false;
  Socket.msg.data.query = (struct ListHead)STAILQ_HEAD_INITIALIZER(Socket.msg.data.query);
...
void ipc_populate_data(char *buf)
{
       ...
       i++, group = strtok_r(NULL, "\x1D", &grest))
  {
         ...
         unit = strtok_r(NULL, "\x1F", &urest))
    {
      ...
      {
        case '\x1':
          mutt_list_insert_tail(&Socket.msg.data.attach, 
                                mutt_str_dup(unit));
          break;
        case '\x2':
          mutt_list_insert_tail(&Socket.msg.data.bcc, 
                                mutt_str_dup(unit));
          break;
        case '\x3':
          mutt_list_insert_tail(&Socket.msg.data.cc, 
                                mutt_str_dup(unit));
          break;
        case '\x4':
          mutt_list_insert_tail(&Socket.msg.data.command, 
                                mutt_str_dup(unit));
          break;
        case '\x5':
          mutt_list_insert_tail(&Socket.msg.data.config, 
                                mutt_str_dup(unit));
          break;
        case '\x6':
          mutt_list_insert_tail(&Socket.msg.data.folder, 
                                mutt_str_dup(unit));
          break;
        case '\x7':
          if (streq(unit, "1"))
          ...
          break;
        case '\x8':
          mutt_list_insert_tail(&Socket.msg.data.query, 
                                mutt_str_dup(unit));
          break;
        case '\x9':
        ...
        case '\x10':
          ...
          mutt_list_insert_tail(&Socket.msg.data.to, 
                                tmp);
          break;
      }
    }
