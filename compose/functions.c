 ...
 */
static int compose_attach_files(struct ComposeSharedData *shared, const char *prompt,
                                int aidx, bool after)
{
  int numfiles = 0;
  char **files = NULL;
      ...
      else
      {
        ...
        if (after)
        {
          ...
          if (bptr_current->parts)
            new_idx += count_attachments(bptr_current->parts, true);
        }
        else
        {
          ap->body->next = bptr_current;
          struct Body *bptr_previous = NULL;
          if (find_body_previous(shared->email->body, bptr_current, &bptr_previous))
          {
          ...
          {
            struct Body *bptr_parent = NULL;
            if (find_body_parent(shared->email->body, NULL, bptr_current, &bptr_parent))
 ...
 */
static int compose_new_mime(struct ComposeSharedData *shared, const char *prompt,
                            int aidx, bool after)
{
  int rc = IR_NO_ACTION;
  struct Buffer *fname = mutt_buffer_pool_get();
  ...
  if ((mutt_buffer_get_field(prompt, fname, MUTT_COMP_FILE, false,
                             NULL, NULL, NULL) != 0) ||
      mutt_buffer_is_empty(fname))
  {
    goto done;
